...
export default function AutoCorrelation() {
  ...
  const [postgresFormat, setPostgresFormat] = useState();
  const [mysqlFormat, setMysqlFormat] = useState();
  ...
  const selectChangeDateOption = (event) => {
    ...
    switch(value) {
      case "Minutes":
        setPostgresFormat("YYYY-MM-DD hh24:MI");
        setMysqlFormat("%Y-%m-%d %H:%i");
        break;
      case "Hour":
        setPostgresFormat("yyyy-mm-dd hh24");
        setMysqlFormat("%Y-%m-%d %H:00");
        break;
      case "Date":
        setPostgresFormat("yyyy-MM-dd");
        setMysqlFormat("%Y-%m-%d");
        break;
    }
  };
  ...
  const handleSubmit = event => {
    ...
    fetch('http://localhost:8000/avanti_dashboard/api/correlation/'+endPoint+'/cross', {
        method: 'POST',
        mode: 'cors',
        cache: "no-cache",
        credentials: "include",
        headers: {'Content-Type': 'application/json'}, 
        body: JSON.stringify({sensorId:sensorId, deviceId:deviceId, startDate:startDate, endDate:endDate, interval:interval, lag:lag, postgresFormat:postgresFormat, mysqlFormat:mysqlFormat}),
      })
